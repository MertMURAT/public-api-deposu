...
async function loadTurkeyFocusedApiIds() {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error("APIs for Türkiye ID file must be a JSON array.");
  }
...
function buildReadme(apis, playgroundApiIds, turkeyFocusedApiIds) {
  ...
  const lines = [
    ...
    `- <a href="${TURKEY_FOCUSED_DOCS_LINK}">${buildTurkeyFocusedIcon(18)} <strong>Browse APIs for Türkiye</strong></a>`,
    `- [Open playground](${siteUrl("/playground")})`,
    `- [Submit an API](${siteUrl("/submit")})`,
    `- [Report an issue](${ISSUES_URL})`,
    `- [Buy Me a Coffee](${donateUrl})`,
    "",
    `APIs marked with ${buildTurkeyFocusedIcon(18)} belong to the curated APIs for Türkiye list.`,
    "",
    "## Categories",
    "",
  ];
...
function buildTurkeyFocusedDocs(apis, playgroundApiIds, turkeyFocusedApiIds) {
  ...
  lines.push("- The main README keeps all categories in one place; this page highlights the APIs for Türkiye slice only.");
...
async function main() {
  ...
  if (missingTurkeyFocusedApiIds.length > 0) {
    throw new Error(
      `APIs for Türkiye IDs not found in live catalog: ${missingTurkeyFocusedApiIds.join(", ")}`,
    );
  }
  ...
  console.log(
    `[generate:readme] README updated with ${apis.length} APIs, ${playgroundApiIds.size} playground links, and ${turkeyFocusedApiIdSet.size} APIs for Türkiye.`,
  );
}
